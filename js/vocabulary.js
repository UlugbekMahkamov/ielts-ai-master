...
async function loadSRSCounts() {
  try {
    const counts = await apiCall('/api/vocabulary/counts');
    document.getElementById('countDue').innerText = counts.due_today || 0;
    document.getElementById('countStage1').innerText = counts.stage_1 || 0;
    document.getElementById('countStage2').innerText = counts.stage_2 || 0;
    document.getElementById('countStage3').innerText = counts.stage_3 || 0;
    document.getElementById('countStage4').innerText = counts.stage_4 || 0;
    document.getElementById('countStage5').innerText = counts.stage_5 || 0;
    document.getElementById('countLearned').innerText = counts.learned || 0;
  } catch (e) {
    console.error(e);
  }
}
...
async function loadWordsForStage(stage) {
  try {
    ...
    allVocabWords = await apiCall(url);
    if (!Array.isArray(allVocabWords)) {
      // Backend real ma'lumot qaytarmadi (masalan, DB ulanishi yo'q) -> zaxira sifatida demo ro'yxat
      const words = getStoredVocabulary();
      allVocabWords = stage === 'due' ? words : words.filter(w => w.interval_stage == stage);
    }
